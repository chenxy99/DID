...
class RFBNet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        #self.trans_layers, self.latent_layers, self.cls_1000 = transfer_block(size=300)
    ...
    def forward(self, x):
        ...
        # apply Imagenet resnet101 KD method
        '''
        if self.size == 300:
            traces.append(self.latent_layers[0](sources[0]))
            for index in range(1, 5):
                traces.append(self.trans_layers[index - 1](self.latent_layers[index](sources[index]) + traces[-1]))
            Imagenet_cls = self.cls_1000(traces[-1].view(-1, 256 * 3 * 3))
        '''
...
def transfer_block(size = 300):
    if size == 300:
        # construct the transfer layer for the unsupervised learning from compared with resnet101
        trans_layers = nn.ModuleList([nn.Sequential(nn.ReLU(inplace=True),
        ...
        latent_layers = nn.ModuleList([nn.Conv2d(512, 256, kernel_size=3, stride=2, padding=1),
                                        nn.Conv2d(1024, 256, kernel_size=3, stride=1, padding=1),
                                        nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1),
                                        nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
                                        nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)])
        cls_1000 = nn.Linear(256 * 3 * 3, 1000)
    elif size == 512:
        pass
    else:
        print("Error: Sorry only SSD300 and SSD512 are supported!")
        return
    return trans_layers, latent_layers, cls_1000
