...
class RFBNet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes, num_classes_target1, num_classes_target2, num_classes_target3, num_classes_target4):
        ...
        self.num_classes_target4 = num_classes_target4 - 1
        self.size = size
        ...
        self.loc_target4 = nn.ModuleList(head[9])
        self.bin_conf_target4 = nn.ModuleList(head[10])
        ...
        self.classifier_target4 = Classifier(self.num_classes - 1, self.num_classes_target4)
        self.scale_target4 = nn.Parameter(torch.FloatTensor([10]))
    ...
    def forward(self, x):
        ...
        # apply multibox head to source layers
        for (x, l, b) in zip(sources, self.loc_target4, self.bin_conf_target4):
            loc_target4.append(l(x).permute(0, 2, 3, 1).contiguous())
            bin_conf_target4.append(b(x).permute(0, 2, 3, 1).contiguous())
        ...
        loc_target4 = torch.cat([o.view(o.size(0), -1) for o in loc_target4], 1)
        bin_conf_target4 = torch.cat([o.view(o.size(0), -1) for o in bin_conf_target4], 1)
        ...
        embedding_target4 = conf.view(-1, self.num_classes)
        # embedding_target1 = embedding_target1[:,1:] -  embedding_target1[:,0:-1]
        embedding_target4 = embedding_target4[:, 1:]
        embedding_target4_norm = self.l2_norm(embedding_target4)
        embedding_target4_output = embedding_target4_norm.view(loc_target4.size(0), -1, self.num_classes - 1)
        # embedding_target1_output = embedding_target1.view(loc_target1.size(0), -1, self.num_classes)
        conf_target4 = self.classifier_target4(embedding_target4_norm)
        conf_target4 = self.scale_target4 * conf_target4
        ...
        if self.phase == "test":
            ...
            output_target4 = (
                loc_target4.view(loc_target4.size(0), -1, 4),  # loc preds
                self.softmax(conf_target4.view(-1, self.num_classes_target4)),  # conf preds
                self.softmax_binary(bin_conf_target4.view(-1, 2)),  # bin_conf preds
            )
        else:
            output = (
            ...
            output_target4 = (
                loc_target4.view(loc_target4.size(0), -1, 4),
                conf_target4.view(loc_target4.size(0), -1, self.num_classes_target4),
                bin_conf_target4.view(loc_target4.size(0), -1, 2),
            )
        return output, output_target1, output_target2, output_target3, output_target4, embedding_target4_output
    ...
    def weight_target4_norm(self):
        w = self.classifier_target4.fc.weight.data
        norm = w.norm(p=2, dim=1, keepdim=True)
        self.classifier_target4.fc.weight.data = w.div(norm.expand_as(w))
...
def multibox(size, vgg, extra_layers, cfg, num_classes, num_classes_target1, num_classes_target2, num_classes_target3, num_classes_target4):
    # for the source
    loc_layers = []
    conf_layers = []
    bin_conf_layers = []
...
#####################################
    ...
    # for the target4
    loc_layers_target4 = []
    bin_conf_layers_target4 = []
    vgg_source = [-2]
    for k, v in enumerate(vgg_source):
        if k == 0:
            loc_layers_target4 += [nn.Conv2d(512,
                                             cfg[k] * 4, kernel_size=3, padding=1)]
            # in this program, I assume that it is a prediction that indicates whether this bounding box is an object
            bin_conf_layers_target4 += [nn.Conv2d(512,
                                                  cfg[k] * 2, kernel_size=3, padding=1)]
        else:
            loc_layers_target4 += [nn.Conv2d(vgg[v].out_channels,
                                             cfg[k] * 4, kernel_size=3, padding=1)]
            # in this program, I assume that it is a prediction that indicates whether this bounding box is an object
            bin_conf_layers_target4 += [nn.Conv2d(vgg[v].out_channels,
                                                  cfg[k] * 2, kernel_size=3, padding=1)]
    i = 1
    indicator = 0
    if size == 300:
    ...
    for k, v in enumerate(extra_layers):
        if k < indicator or k % 2 == 0:
            loc_layers_target4 += [nn.Conv2d(v.out_channels, cfg[i]
                                             * 4, kernel_size=3, padding=1)]
            # in this program, I assume that it is a prediction that indicates whether this bounding box is an object
            bin_conf_layers_target4 += [nn.Conv2d(v.out_channels, cfg[i]
                                                  * 2, kernel_size=3, padding=1)]
            i += 1
    ...
    return vgg, extra_layers, (loc_layers, conf_layers, bin_conf_layers, loc_layers_target1, bin_conf_layers_target1, loc_layers_target2, bin_conf_layers_target2,
                               loc_layers_target3, bin_conf_layers_target3, loc_layers_target4, bin_conf_layers_target4)
...
def build_net(phase, size=300, num_classes=21, num_classes_target1 = 21, num_classes_target2=6, num_classes_target3=6, num_classes_target4=6):
    ...
    return RFBNet(phase, size, *multibox(size, vgg(base[str(size)], 3),
                                ...
                                mbox[str(size)], num_classes, num_classes_target1,
                                         num_classes_target2, num_classes_target3, num_classes_target4),
                  num_classes, num_classes_target1, num_classes_target2, num_classes_target3, num_classes_target4)
